...
export class App{
	...
	_init():void{
		...
		this.drawGrid(21,true)
     	this._buildBox(new Vector3(0,0,1));
         this._addControls(); //@ anything that requires render.dom must call after the Render.dom is created.
		this._addLight();
		
		window.addEventListener('resize', this._resize.bind(this) );
		window.addEventListener("DOMContentLoaded",()=>{
			let vbutton:HTMLElement = document.querySelector("#VRButton");
			let timeLine:HTMLElement = document.querySelector('#timeline');
			let height = timeLine.offsetHeight + 1;
            vbutton.style.position='absolute';
			
			vbutton.style.bottom=`${height}px`;
			
			// vbutton.style.background='#ff0000';
		})
	...
	private _buildBox(location?:Vector3):void
	{
        ...
        const material:Material = new MeshStandardMaterial({color: new THREE.Color( 0xffffff ) })
	...
	private _addLight(){
		let ambient:AmbientLight = new AmbientLight(new Color('#ffffff'),0.2);	this.scene.add( ambient );
